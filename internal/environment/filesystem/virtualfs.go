...
func (self *VirtualFS) ToFullPath(file *storage.FileUrl) (string, error) {
	// path inside of root
...
func (self *VirtualFS) addFolder(url *storage.FileUrl) (*fs.Inode, error) {
	p := &self.root.Inode
...
func (self *VirtualFS) addNode(url *storage.FileUrl, node fs.InodeEmbedder) error {
...
type vfsInputFile struct {
	...
	input *storage.FileUrl

	mu      sync.Mutex
	storage storage.Storage
	reader  storage.VirtualFileReader
	offset  int64
}
...
func (self *vfsInputFile) ensureConnection() error {
	if self.storage == nil {
		storage, err := storage.ConnectStorage(self.input)
		if err != nil {
...
type vfsOutputFile struct {
	...
	output *storage.FileUrl

	mu      sync.Mutex
	storage storage.Storage
	writer  storage.VirtualFileWriter
	offset  int64
}
...
func (self *vfsOutputFile) ensureConnection() error {
	if self.storage == nil {
		storage, err := storage.ConnectStorage(self.output)
		if err != nil {
			return fmt.Errorf("Unable to connect to storage: %s", err)
		}
		self.storage = storage
